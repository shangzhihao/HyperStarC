...
from .erlang_handler import er_fit_md_change, er_max_phase_change, er_round_change
from .fit_handler import fit_click, fitter_change
from .herlang_handler import (
    her_fit_md_change,
    her_max_phase_change,
    her_peaks_change,
    her_round_change,
)
from .map_handler import (
    map_fit_md_change,
    map_max_phase_change,
    map_peaks_change,
    map_round_change,
)
from .plot_handler import bins_num_change, max_x_change, min_x_change, replot_click
...
with page:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            max_x = gr.Number(
                value=params.value.draw_max_x,
                label="max x for plotting",
                interactive=True,
            )
            min_x = gr.Number(
                value=params.value.draw_min_x,
                label="min x for plotting",
                interactive=True,
            )
            sample_num = gr.Number(
                value=1000, label="number of samples for plotting", interactive=True
            )
            ...
            with gr.Row(visible=False) as erlang_block:
                ...
                er_max_phase = gr.Number(
                    value=config.default_param.erlang_max_phase,
                    label="max phase",
                    interactive=True,
                )
            with gr.Row(visible=False) as hyper_block:
                her_peaks = gr.Number(
                    value=config.default_param.herlang_peaks,
                    label="peaks",
                    interactive=True,
                )
                ...
                her_max_phase = gr.Number(
                    value=config.default_param.herlang_max_phase,
                    label="max phase",
                    interactive=True,
                )
            with gr.Row(visible=False) as map_block:
                ...
                map_fit_md = gr.Dropdown(
                    config.ERMD_NAMES, label="method", interactive=True
                )
                map_round = gr.Dropdown(
                    config.RUNDING_NAMES, label="rounding", interactive=True
                )
                map_max_phase = gr.Number(
                    value=config.default_param.herlang_max_phase,
                    label="max phase",
                    interactive=True,
                )
    ...
    load_btn.upload(
        fn=upload_samples,
        inputs=[load_btn, params],
        outputs=(pdf_plot, cdf_plot, params),
    )
    ...
    er_max_phase.change(
        fn=er_max_phase_change, inputs=[er_max_phase, params], outputs=params
    )
    ...
    her_max_phase.change(
        fn=her_max_phase_change, inputs=[her_max_phase, params], outputs=params
    )

    map_peaks.change(fn=map_peaks_change, inputs=[map_peaks, params], outputs=params)
    map_fit_md.change(fn=map_fit_md_change, inputs=[map_fit_md, params], outputs=params)
    map_round.change(fn=map_round_change, inputs=[map_round, params], outputs=params)
    map_max_phase.change(
        fn=map_max_phase_change, inputs=[map_max_phase, params], outputs=params
    )
